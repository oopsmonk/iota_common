...
void tx_inputs_print(tx_inputs_t *tx_in) {
  ...
  char output_id_str[OUTPUT_ID_STR_LEN];
  size_t id_len = OUTPUT_ID_STR_LEN;
  printf("inputs:[\n");
  TX_INPUTS_FOREACH(tx_in, output_id) {
    tx_output_id_2_string(output_id_str, &id_len, output_id);
    printf("  %s\n", output_id_str);
  }
  printf("]\n");
}
...
void tx_outputs_print(tx_outputs_t *tx_out) {
  char addr[IOTA_ADDRESS_STR_LEN];
  char color[BALANCE_COLOR_STR_LEN];
  tx_output_t *o = NULL;
  printf("inputs:[\n");
  TX_OUTPUTS_FOREACH(tx_out, o) {
    address_2_string(addr, o->address);
    printf("  %s: [\n", addr);
    balance_color_2_string(color, o->balance.color);
